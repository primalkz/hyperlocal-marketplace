...
function assertActive(shop: { status: string }) {
  if (shop.status === 'DISABLED' || shop.status === 'REJECTED') throw new AppError(403, 'shop not active')
}
...
router.patch('/shop', validate(shopSchema), async (req, res, next) => {
  ...
  assertActive(shop)
  const { name, latitude, longitude } = req.body
  await prisma.shop.update({ where: { id: shop.id }, data: { name, latitude, longitude } })
  res.json({ shop: await shopOf(req.user!.userId) })
})
