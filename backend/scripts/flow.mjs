...
const v3 = new Sess()
await v3.post('/auth/register', { email: 'v3@t.test', password: 'secret123', role: 'VENDOR' })
await v3.post('/vendor/shop', { name: 'Quick Mart', latitude: 19.04, longitude: 72.84 })
await admin.patch('/admin/vendors/' + (await v3.get('/auth/me')).body.user.userId, { status: 'DISABLED' })
r = await v3.post('/vendor/products', { title: 'Milk 1L', image: 'https://picsum.photos/seed/milk/400', price: 50, available: true })
console.log('disabled vendor add product (expect 403):', r.status, r.body?.error)
